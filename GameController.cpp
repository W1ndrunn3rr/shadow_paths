...
void GameController::handleGameEvent(sf::Event &event, sf::RenderWindow &window) {
    ...
    if (event.type == sf::Event::MouseButtonPressed && !isMouseClicked) {
        if (event.mouseButton.button == sf::Mouse::Left) {
            ...
            if (status == HTP && getMenuMousePosition() == HOW_TO_PLAY_EXIT_POSITION) {
                music.playClick();
                status = MENU;
            }
            if (status == IN_GAME) {
                ...
                if (location == BONFIRE && getNPCMousePosition() == CHOOSE_POSITION && quest.getBoss() != CLEARED) {
                ...
                } else if (questLocation == COMPLETED && getNPCMousePosition() == CLOSE_POSITION) {
                    music.playClick();
                    questLocation = NOT_IN_QUEST;
                    music.playFirePlace();
                }

                if (location == DUNGEON && getDungeonMousePosition() == DUNGEON_FIGHT_POSITION &&
...
GameController::GameController(Player &player, Merchant &merchant, GameView &view, Quest &quest,Music &music) : player(player),
                                                                                                   merchant(merchant),
                                                                                                   view(view),
                                                                                                   quest(quest),music(music) {
    questLocation = NOT_IN_QUEST;
    status = MENU;
    location = BONFIRE;
    dgStatus = NOT_IN_DUNGEON;
...
MenuMousePosition GameController::getMenuMousePosition() const {
    ...
    else if (mousePosition.x > 1138 && mousePosition.x < 1242 && mousePosition.y > 650 &&
             mousePosition.y < 780)
...
InGameMenuMousePosition GameController::getInGameMenuMousePosition() const {
    if (mousePosition.x > 55 && mousePosition.x < 347) {
        ...
        } else if (mousePosition.x > 325 && mousePosition.x < 378 && mousePosition.y > 72 &&
                   mousePosition.y < 139)
...
BlacksmithMousePosition GameController::getBlacksmithMousePosition() const {
    ...
    } else if (mousePosition.x > 1018 && mousePosition.x < 1194 && mousePosition.y > 457 &&
               mousePosition.y < 628) {
...
NPCMousePosition GameController::getNPCMousePosition() const {
    ...
    } else if ((mousePosition.x > 393 && mousePosition.x < 444 && mousePosition.y > 534 &&
                mousePosition.y < 586) || (mousePosition.x > 595 && mousePosition.x < 688 &&
                                           mousePosition.y > 525 && mousePosition.y < 618)) {
        return CLOSE_POSITION;
    } else if (mousePosition.x > 1142 && mousePosition.x < 1280 && mousePosition.y > 734 &&
               mousePosition.y < 800) {
...
DungeonPosition GameController::getDungeonMousePosition() const {
    if (mousePosition.x > 773 && mousePosition.x < 894 &&
        mousePosition.y > 649 &&
        mousePosition.y < 762) {
        return DUNGEON_FIGHT_POSITION;
    }

    if (mousePosition.x > 595 &&
        mousePosition.x < 688 &&
        mousePosition.y > 525 && mousePosition.y < 618)
