...
void Merchant::buyPotion() {
    if(player.getGold() >= potionGoldRequired && (player.getPotions() < 1*player.getPotionsLevel())) {
        player.getPotion();
...
int Merchant::getBowGoldRequired() const {
    return weaponGoldRequired;
}

int Merchant::getArmorGoldRequired() const {
    return armorGoldRequired;
}
