...
void GameView::setMenuFont(sf::RenderWindow &window) {
    ...
    if (mousePosition.x > 450 && mousePosition.x < 828 && mousePosition.y > 425 && mousePosition.y < 547)
    ...
    if (mousePosition.x > 550 && mousePosition.x < 728 && mousePosition.y > 580 && mousePosition.y < 671)
...
void GameView::setGameFont(sf::RenderWindow &window) {
    ...
    if (mousePosition.x > 55 && mousePosition.x < 347 &&
        mousePosition.y > 162 && mousePosition.y < 262)
    ...
    if (mousePosition.x > 55 && mousePosition.x < 347 &&
        mousePosition.y > 292 && mousePosition.y < 392)
    ...
    if (mousePosition.x > 55 && mousePosition.x < 347 &&
        mousePosition.y > 422 && mousePosition.y < 522)
    ...
    if (mousePosition.x > 55 && mousePosition.x < 347 &&
        mousePosition.y > 552 && mousePosition.y < 652)
    ...
    if (mousePosition.x > 55 && mousePosition.x < 347 &&
        mousePosition.y > 682 && mousePosition.y < 782)
...
void GameView::drawAlchemist(sf::RenderWindow &window) {
    ...
    aPotion.setString("Max potions: " + std::to_string(1 * player.getPotionsLevel()));
...
void GameView::drawCharacter(sf::RenderWindow &window) {
    ...
    stats1.setString("Health : " + std::to_string(player.getCurrentHealth()) + "\n\n" + "Armor: " +
                     std::to_string(player.getArmor()) + "                               "  "\n\n" + "Damage: " +
                     std::to_string(player.getMinDamage()) + "-" + std::to_string(player.getMaxDamage()) + "~(" +
    ...
    stats2.setString("Experience points: " + std::to_string(player.getExperiencePoints()) +
                     "\n\n" + "Shards: " + std::to_string(player.getShards()) + "\n\n" + "Potions: " +
                     std::to_string(player.getPotions()) + "\n\n"
                                                           "Armor level: " + std::to_string(player.getArmorLevel()));
...
void GameView::drawQuestText(sf::RenderWindow &window) {
    ...
    questOne.setString(quest.getQuestName() + '\n' + '\n' + quest.getQuestContent() + "\n" + "Gold: " +
                       std::to_string(quest.getGoldAmount())
...
void GameView::drawQuest(sf::RenderWindow &window) {
    ...
    sf::Time countdownTime = sf::seconds(float(quest.getDuration())); // do zmiany na quest.getDuration();
...
void GameView::drawFight(Enemy &enemy, sf::RenderWindow &window, BossType type, int mode) {
    ...
    sf::Sprite fightOne(bansheeFight), fightTwo(kingFight), fightThree(watchersFight);
    ...
    float enemyHealthRatio = static_cast<float>(enemy.getCurrentHealth()) / float(enemy.getMaxHealth());
    float playerHealthRatio = static_cast<float>(player.getCurrentHealth()) / float(player.getMaxHealth());

    if ((enemyHealthRatio >= 0.0 && enemyHealthRatio < 0.16) || enemyHealthRatio < 0)
        enemyChoice = 0;
    ...
    if ((playerHealthRatio >= 0.0 && playerHealthRatio < 0.16) || playerHealthRatio < 0)
        playerChoice = 0;
...
//Funkcja napisana za pomocą Chatu GPT -obsługa animacji ataku gracza (strzały) i wyświetlanie na ekranie
void GameView::drawPlayerAttack(sf::RenderWindow &window, float deltaTime, float initialPosX, Enemy &enemy,
                                BossType type) {
...
void GameView::drawMonsterAttack(sf::RenderWindow &window, Enemy &enemy, BossType type) {
...
void GameView::drawBossAbility(sf::RenderWindow &window, Boss &boss, BossType type) {
